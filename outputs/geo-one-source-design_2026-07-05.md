...
4b. **Multi-polygon zones (added 2026-07-05, Eran — timing.tech reference):** timing.tech's territory UI supports several adjustable polygons per area incl. sub-city splits (their screenshots show ~10 rings around one city with km² labels). Our schema already supports it (`zones.polygons` is an array and `resolveZone` matches ANY ring) — but `confirmZoneDraw` *replaces* the array with the single drawn ring. Slice A makes draw **append**: existing rings render on the draw map, a new ring is added (with a replace option), so a client can divide one city into several polygons. Vertex-editing of saved rings + km² labels = later enhancement (logged in backlog).
