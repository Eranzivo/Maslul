...
## Interpretation (verified findings; identical before/after the review fixes)
...
4. **Existing calls untouched.** 0 retimes proposed; 0 write patches — dry-run is airtight.
...
6. **Balance-on spread is visible** in Run B (calls thinly spread across two weeks) — the live `balance.enabled:true` semantics; consolidate-vs-spread (placement-policy unification, Slice 3) still pending with Israel.
