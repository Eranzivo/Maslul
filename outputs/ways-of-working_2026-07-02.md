...
**Still open:** leaked-password protection (dashboard toggle — not settable via SQL); drop stale `tasks_backup_20260613` (needs Eran's confirmation it's no longer needed); RLS-perf batch from the 06-12 review (wrap `auth.uid()`, index tenant FKs, consolidate permissive policies).
