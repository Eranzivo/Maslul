...
- 🔴 **Global `L` is Leaflet. The app's label helper is `LBL(key)`, never `L`.** Leaflet exports a global `L`, and **leaflet-draw references it at runtime** (its hooks do `L.*`), so Leaflet must own `L`. The i18n label helper is therefore named `LBL(key)` (was `L(key)` — renamed 2026-06-11). `LF` aliases Leaflet (`window.LF = window.L`) for the map code. **Rules:** labels → `LBL('worker')`; Leaflet → `LF.map`/`LF.marker`/`LF.Control.Draw` (or plain `L`, same object); **never name anything `L`.** History: an earlier `noConflict()` fix freed `L` for the label helper but broke leaflet-draw ("Cannot read properties of undefined (reading 'on')" in `addHooks`) — the rename is the root fix.
