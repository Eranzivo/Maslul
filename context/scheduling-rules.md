...
- **Editable (Phase 3, June 2026):** two complementary paths —
  ...
  - *Out-of-zone safeguard (`scheduling.zone_drop_guard`, default ON for zone-mode tenants; set `false` to opt out):* all three placement paths (`_onCellDrop`, `_onGridDrop`, `placeTaskDetail`) call `confirmZoneDrop` first. If the call's city resolves to a **different** zone than the tech's assigned zone that day (`zoneDropMismatch`), a confirm warns ("שיבוץ רחוק מהאזור עלול להאריך מאוד את המסלול") and aborts on decline — **fail-soft**, the coordinator may still proceed. Out-of-zone calls show a ⚠ in both planner views (`taskOutOfZone`, render-time, no persisted flag, auto-clears when moved back in-zone). Unknown-zone cities (e.g. חרב) are left to needs_location, not flagged here.
