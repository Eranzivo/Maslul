...
| **`auto_overrun_min`** (default 15) | Eran 2026-07-12: minutes a new placement may let service spill past the promised window end under `arrive` semantics; beyond ⇒ next window/day; 0 = strict avoid. **Both doors share the tolerance (unified 2026-07-13):** the automatic (batch) path books ≤tol spills silently; the LIVE dispatch popup fires only when the spill EXCEEDS the tolerance (small spills book silently there too), so the two doors behave alike. `finish` semantics ⇒ effective tolerance 0 (any spill asks/defers). Existing calls' promises never re-broken (sequencer unchanged) | `resolveAutoOverrunMin` + `overrunMinutes`/`overrunDecision` (dispatch popup gate passes the tolerance) | `resolve_auto_overrun_min` + `narrow_window_for_overrun` (pref-window new calls, solver-hard) + `promote_spilled_window` (derived-window promise, fail-open); `overrun_decision` mirrors the JS gate | ✅ via narrowed window_end | overrun-cases.json BOTH suites + test_overrun.py (11) |
...
| **`window_semantics`** (finish/arrive, default finish) | what the customer window PROMISES (Eran 2026-07-11): `finish` = job must END inside the window (conservative default for new tenants); `arrive` = tech must ARRIVE by window end, service may overrun — opens the window's last [duration] minutes for booking (~17% more arrival capacity). **PureWater = arrive** (Israel's real operation). 10–15 min overrun is deliberately NOT surfaced in UI. route_health audits with arrival semantics regardless (lenient — never false-flags) | `resolveWindowSemantics` — ships the knob in every optimize payload (`_postOptimize`, `shadowCompare`) | `resolve_window_semantics` → `solve_route_v2(window_semantics=…)` (the enforcement point; batch + /optimize + /audit-day + nightly sweep all pass it) | ✅ hi-bound branch | sched.test.js resolver + test_window_semantics.py (resolver parity + solver behavior both values) |
...
|---|---|---|---|---|---|
| **`routing.traffic_mode`** (off/rush_hour/live, default off) | time-of-day travel bucketing for the global route brain. `off` (PureWater) ⇒ always `static` bucket ⇒ zero behavior change; `rush_hour` ⇒ `rush` inside 07-09/16-18 else `static`; `live` ⇒ `live`. Maps departure time → `route_cache.time_bucket` (`route_cache.py` now bucket-aware, default static). Observations logged per-bucket (P1); time-dependent matrix routing = future. Vision: [[cross-tenant-brain]] / `outputs/cross-tenant-brain-design_2026-07-14.md` | `resolveTrafficMode` + `trafficBucket` | `resolve_traffic_mode` + `traffic_bucket` | n/a yet (time-dependent matrix = future) | traffic-cases.json BOTH suites (sched.test.js + test_traffic_parity.py) |
| **`routing.learned_durations`** (bool, default off) | MASTER switch for using learned travel times. **APPROVAL-GATED (Eran 2026-07-14): learned data NEVER auto-applies** — even ON, `/optimize` uses only legs a human APPROVED in 🧠 מוח המערכת (`route_learned_approved`), never raw observations. OFF ⇒ feature dormant. **Backend-only** (matrix building is server-side); default OFF ⇒ every current tenant unchanged; fail-open. | n/a **by design** (live JS builds no matrix — backend `/optimize` owns travel) | `resolve_learned_durations` → `route_observations.get_learned_legs` (reads `route_learned_approved`) → `build_matrix_cached(learned_legs=)` | approved legs win in the matrix | test_route_observations.py + test_matrix_cached.py (override + None-unchanged) |
...
| Knob | Meaning | Live JS | Batch/Py | Solver | Test |
|---|---|---|---|---|---|
| `reports.cards {key:bool}` (default all visible) | which report cards a tenant sees: `kpis` / `team` / `zones` / `categories` / `weekday` / `insights` — only an explicit `false` hides (Eran's per-tenant-cards rule). **Settings UI:** 📊 דוחות ותובנות section (2026-07-12) | `resolveReportCards` (gates every card in `renderReports`) | n/a **by design** (display-only) | n/a | sched.test.js `resolveReportCards` |
| `insights.window_days` (default 90) | how far back demand counts look (zone cards' ביקוש line; the zones timeframe chip). Settings UI: same section | `resolveInsightsWindow` (`renderZones` demand cutoff) | n/a **by design** (display-only) | n/a | sched.test.js `resolveInsightsWindow` |
| _duration-accuracy insight_ (NOT a knob — named constants) | E4-lite: per category, configured vs ACTUAL service time (`completed_at−arrived_at`) as a reports suggestion. Thresholds `DURACC_MIN_SAMPLE=3` / `DURACC_DELTA_PCT=0.25` are display constants, identical for every tenant, change no engine behavior ⇒ deliberately not a per-tenant knob (promote to `insights.duration_*` only if a 2nd tenant needs different sensitivity). | `durationAccuracyInsights` (in `renderReports` insights) | n/a **by design** (display-only; influences no assignment) | n/a | sched.test.js `durationAccuracyInsights` (golden cases) |
