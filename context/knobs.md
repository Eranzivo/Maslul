...
| **`auto_overrun_min`** (default 15) | Eran 2026-07-12: minutes an UNATTENDED new placement (batch — no coordinator) may let service spill past the promised window end under `arrive` semantics; beyond ⇒ next window/day; 0 = strict avoid. The LIVE door never books an overrun silently — coordinator popup (`overrunDecision`='ask'). Existing calls' promises are never re-broken (sequencer unchanged) | `resolveAutoOverrunMin` + `overrunMinutes`/`overrunDecision` (dispatch popup door) | `resolve_auto_overrun_min` + `narrow_window_for_overrun` (pref-window new calls, solver-hard) + `promote_spilled_window` (derived-window promise, fail-open) | ✅ via narrowed window_end | overrun-cases.json BOTH suites + test_overrun.py (11) |
