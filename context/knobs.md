...
| **`routing.learned_durations`** (bool, default off) | P2 consumption: when ON, `/optimize` prefers the tenant's OWN observed leg medians (`route_observations`, ≥3 samples) over cache/Google/haversine. **Backend-only** (matrix building is server-side); default OFF ⇒ every current tenant unchanged; fail-open. | n/a **by design** (live JS builds no matrix — backend `/optimize` owns travel) | `resolve_learned_durations` → `route_observations.get_learned_legs` → `build_matrix_cached(learned_legs=)` | learned legs win in the matrix | test_route_observations.py + test_matrix_cached.py (override + None-unchanged) |
