...
- [ ] **Configurable working days / blocked days per tenant (#2026-06-15)** — easy UI to set each tenant's operating week so calls NEVER land on off-days. PureWater = Sun–Thu (Fri+Sat off, always). Today Saturday is hardcoded-excluded in `batch_schedule.py` and Friday depends on whether `rotation`/`weekly_schedule` happens to cover it — too implicit. Make it explicit per-tenant config (`scheduling.working_days` / `blocked_days`), honoured by BOTH the batch engine and the live `_candidatesZone` path, and surfaced in the tenant/tech settings UI. Generic (each client sets their own week); absent config = today's behaviour. Eran flagged 2026-06-15.
