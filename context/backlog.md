...
> **⏸ PAUSED (2026-06-15) — PureWater zone/scheduling deep-dive** ("still seems off, get back to it"). Full state + resume plan: `outputs/zone-gap-analysis_2026-06-15.md`. TL;DR: far→near backtrack FIXED in engine (`995bc28`, committed not deployed); 20 pending = capacity (תל אביב 27/לוד-אשדוד 25/דרום 19 vs 2 covering days × max_daily 9) + חרב(no coords) + סלעית(wrong zone); 6 orphan duplicate zones to delete. Resume: delete orphans → fix סלעית → Israel decides 3rd-covering-day vs spill → deploy fix → dry-run re-batch.
