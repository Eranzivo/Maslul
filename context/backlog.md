...
## 🟠 Next
- [ ] **Out-of-zone drop safeguard** — a coordinator can drag a far city (e.g. תל אביב) into a South-zone day; the optimizer sequences by real coords, so the ~100 km detour blows the day's drive budget (long day or dropped calls). **Design (fail-soft, per product philosophy — warn, don't block):** on drop/place, resolve the call's zone and compare to the tech's assigned zone for that day (`tech.rotation[dow]` for PureWater); if they differ, show a confirm ("העיר X רחוקה מאזור היום — עלול להאריך מאוד את המסלול. להמשיך?"). On confirm, allow it but tag the call with a ⚠ out-of-zone flag visible in the planner. Tenant-configurable (`scheduling.zone_drop_guard`), default on for zone-mode tenants. Hook points: `_onCellDrop`, `_onGridDrop`, `placeTaskDetail`. Reuse `resolveZone()`. *(raised 2026-06-13 during calendar QA)*
