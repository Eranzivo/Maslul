...
| E5 | Manual drag onto a day that can't absorb more work | zone-checked + **capacity-checked** (`confirmCapacityDrop` on all 3 manual paths, 2026-07-07): warns when the day is at the tech's max daily load, or hard-blocks under `route_strict`. Deeper travel-time-infeasibility (vs. pure count) still deferred | knob `route_strict` |
