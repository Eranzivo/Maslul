...
| B5 | **Window semantics DECIDED (Eran 2026-07-11):** the customer window promises ARRIVAL for PureWater — `scheduling.window_semantics: arrive` (new knob, default `finish` for new tenants). Solver books arrivals until window end; service may overrun; 10–15 min overrun is fine and NOT surfaced in UI. Rationale: "book those 30 min every window last half hour… Israel will change it if needed." Enforced: `solve_route_v2(window_semantics)` both doors; tests test_window_semantics.py + sched.test.js | ✅ shipped 2026-07-11 |
