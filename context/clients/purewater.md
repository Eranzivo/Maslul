...
| `scheduling.equal_city_distribution` | `true` (legacy, inert) | **Superseded by `placement_policy` (Slice 3, 2026-07-06):** under `consolidate` it only acts as the legacy same-city tie-breaker in live dispatch. Safe to leave; remove at next config cleanup. |
| `scheduling.balance.enabled` | `true` (legacy, inert) | **Superseded by `placement_policy` (Slice 3, 2026-07-06):** the old cross-engine contradiction (2026-06-29 finding) is dead — both doors resolve through one policy, and the explicit `consolidate` outranks this flag. Safe to leave; remove at next config cleanup. |
