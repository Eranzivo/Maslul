...
| `scheduling.auto_overrun_min` | absent ⇒ **default 15** (Eran 2026-07-12: "book overrun up to 15 minutes, if more → next window/day") | Automatic (batch) placements may spill ≤15 min past window end; beyond → next-best window/day. Live dispatch always pops the overrun confirmation regardless. See `context/scheduling-rules.md` → Window Overrun Policy. |
...
| Date | Change |
|---|---|
| 2026-07-15 | **9 out-of-zone settlements seeded into zones** (geo self-healing test — the Health tile surfaced them). Coordinate-driven nearest-zone-member: **דרום** ×4 (כפר אחים, קיבוץ שובל, כפר מימון, בני דקלים — last two → דרום per Eran, coord said שפלה), **שרון זכרון עד הרצליה** ×3 (יקנעם, כפר נטר, בארותיים), **צפון מזרח כנרת וגליל** ×1 (בלפוריה), **שפלה אשקלון ומודיעין** ×1 (כפר בן נון). All already had coords in `geo_places`; only zone membership added (additive, reversible via `array_remove`). Geo Health out_of_zone **9→0**; חרב stays unresolved (junk city on a task, not a zone gap). Validated the geo-selfheal loop end-to-end via MCP. |
| 2026-07-06 | **`placement_policy: consolidate` written** (Eran approved after Israel's handover settled Scenario D). One `jsonb_set` via MCP, readback-verified. Both doors now consolidate identically; legacy `balance.enabled`/`equal_city_distribution` flags left in place but outranked (inert). Same day: national OSM import benefits PureWater directly — polygon zone drawing now sees all 1,310 settlements (Eran QA'd on live map). |
