...
- **Slice 3 (backend endpoint, committed):** POST `/geo-suggest` `{city}` → `resolve_or_suggest` over the brain's `place_keys()`; valid session required (global PII-free geo, no tenant logic); fail-open ⇒ `{status:'fail'}`. **Door wiring pending** — the doors already have a WEAKER JS fuzzy layer (`canonicalCity`/`isKnownCity`); Slice 3 upgrades those to this endpoint, flag-gated `features.geo_suggest` (default off), auto-fix the FIELD only (no brain writes — those need coordinator approval via the future P2 page).
