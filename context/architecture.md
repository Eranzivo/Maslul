...
- **Slice 2 (backend engine, committed):** `backend/geo_suggest.py` — `resolve_or_suggest`/`suggest`/`levenshtein`, confidence tiers (resolved / suggest+`auto_ok` / fail). Backend-only (single authority, NO JS mirror). Golden fixture `tests/fixtures/geo-suggest-cases.json`.
- **Slice 3 (endpoint + doors, committed):** POST `/geo-suggest` `{city}` → `resolve_or_suggest` over the brain's `place_keys()`; valid session required (global PII-free geo, no tenant logic); fail-open ⇒ `{status:'fail'}`. **Doors wired, flag-gated `features.geo_suggest` (default OFF ⇒ dormant, zero behavior change):** `geoSuggestCity()` is called only in each door's *existing unknown-city branch* — dispatch (`_trySuggestThenSearch`→re-search once, `_noSuggest` loop-guard), add-task (`confirmAddTask` async), bulk-import (AUTO-fix only, no per-row prompts), zone-authoring (`confirmAddCity` async, backend fallback after the local `canonicalCity`/`isKnownCity` layer). Auto-fix the FIELD only; **no brain writes** (aliases/places need coordinator approval via the future P2 page). Auto (`auto_ok`) applies silently w/ toast; lower-confidence prompts "האם התכוונת ל-X?"; `fail` keeps today's behavior. **Interactive — needs browser QA before flag-on.**
- **Slice 4 (add-city→zone, committed):** POST `/suggest-zone` `{tenant_id,city}` → the tenant's zone whose NEAREST member is closest (pure `geo_suggest.nearest_zone` + `_haversine_km`, 3 tests). The add-city modal (`suggestZoneForAddCity` on open + on city `onchange`) pre-selects that zone + shows a 🎯 hint (`#ac-zone-hint`), coordinator can override; flag-gated `features.geo_suggest`, fail-open (unresolvable city / backend down ⇒ manual pick). This is the self-serve version of the 2026-07-15 SQL seeding of the 9 settlements. **Not yet done (Slice 4b):** persisting a brand-NEW city's geocoded coords into `geo_places` (needs a backend write endpoint; deferred — writing the global shared brain warrants its own careful/approved handling).
- **Slice 5 (partial, 2026-07-15):** the 9 out-of-zone settlements seeded into PureWater zones by hand (coordinate-driven nearest-member, MCP). Health tile out_of_zone 9→0. See `context/clients/purewater.md`.
- Roadmap (design `outputs/geo-selfheal-design_2026-07-14.md`): Slice 4b (persist new-city coords) → P2 approval page (brain-write governance). Learned aliases stay **approval-gated** (doctrine).
